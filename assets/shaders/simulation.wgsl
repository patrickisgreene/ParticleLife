...
struct Params {
    ...
    sampling: vec4<f32>, // neighbor sample budget, unused, unused, unused
}
...
fn interact(a: Particle, b: Neighbor, row: u32, successor: u32, cached: bool) -> Response {
    ...
    out.nearby = 1.0;
    if b.kind == successor { out.successors = 1.0; }
    if distance <= 0.00001 { return out; }
    ...
    // counterclockwise motion around that neighbor. Suppress in the core.
...
fn update(@builtin(global_invocation_id) id: vec3<u32>, @builtin(local_invocation_index) lane: u32) {
    ...
    destination[i] = Particle(pos, velocity, kind, age, 0u, 0u);
    if p.trails.x > 0.0 {
