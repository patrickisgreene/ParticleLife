...
struct Params { counts: vec4<u32>, physics: vec4<f32>, world: vec4<f32>, view: vec4<f32>, timing: vec4<f32>, flags: vec4<u32>, trails: vec4<f32>, trail_view: vec4<f32>, behavior: vec4<f32>, cycle: vec4<f32>, sampling: vec4<f32>, brush: vec4<f32>, interaction: vec4<f32>, nova: vec4<f32> }
...
// Remaining burn time from Particle.pad1, normalized: 1 at ignition, 0 when spent
// and for every particle while the nova rule is off (the pad is zero everywhere).
fn flare_of(particle: Particle) -> f32 {
    if p.nova.w <= 0.0 { return 0.0; }
    return clamp(bitcast<f32>(particle.pad1) / p.nova.w, 0.0, 1.0);
}
const FLARE_GAIN: f32 = 8.0;
const FLARE_SWELL: f32 = 1.5;
...
fn splat(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let c = b.kind * 4u;
    let flare = flare_of(b);
    var tint = vec3(types[c], types[c + 1u], types[c + 2u]);
    tint *= (1.0 + p.timing.y * 2.0) * (1.0 + flare * FLARE_GAIN);
    ...
    let radius = max(p.view.w * (1.0 + flare * FLARE_SWELL) / p.world.x * p.world.w * viewport.y, 0.0001);
...
// Draw after particles so the tool footprint stays visible over dense clusters.
@fragment
fn interaction_fragment(in: VertexOut) -> @location(0) vec4<f32> {
    let world = (in.uv * vec2(p.view.x, 1.0) / p.world.w + p.view.yz + 1.0) * p.world.x * 0.5;
    let pixel = p.world.x / max(1.0, p.sampling.w) / p.world.w;
    var color = vec3(0.0);
    if p.interaction.x != 0.0 {
        var delta = world - p.brush.xy;
        delta -= round(delta / p.world.x) * p.world.x;
        let distance = length(delta);
        let ring = 1.0 - smoothstep(pixel * 0.7, pixel * 2.0, abs(distance - p.brush.z));
        let fill = select(0.0, 0.025, distance < p.brush.z);
        var tint = select(vec3(1.0, 0.35, 0.2), vec3(0.25, 0.8, 1.0), p.interaction.x > 0.0);
        if p.interaction.x == 2.0 { tint = vec3(0.35, 1.0, 0.5); }
        color += tint * (ring * 0.7 + fill);
    }
