...
impl Plugin for ParticleGpuPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins((
            ...
            ExtractResourcePlugin::<InteractionTools>::default(),
...
impl Params {
    fn with_tools(mut self, tools: &InteractionTools) -> Self {
        if let Some(cursor) = tools.cursor {
            self.brush = Vec4::new(
                cursor.x,
                cursor.y,
                tools.radius,
                if tools.active {
                    tools.strength * tools.tool.brush_sign()
                } else {
                    0.
                },
            );
            self.interaction.x = if tools.tool == Tool::Dump {
                2.
            } else {
                tools.tool.brush_sign()
            };
        }
        self
    }
    ...
    fn new(s: &Simulation, a: &Appearance, alpha: f32, generation: u64) -> Self {
        ...
        Self {
            ...
            // A zero threshold disables ignition, the same way a zero gain
            // disables the density term below.
            nova: Vec4::new(
                if s.behavior.nova_enabled {
                    s.behavior.nova_threshold
                } else {
                    0.0
                },
                s.behavior.nova_radius,
                s.behavior.nova_impulse,
                s.behavior.nova_duration,
            ),
...
struct Pipelines {
    ...
    interaction: CachedRenderPipelineId,
}
fn buffer_entry(
    binding: u32,
...
fn setup_pipelines(mut commands: Commands, assets: Res<AssetServer>, cache: Res<PipelineCache>) {
    ...
    resolve_descriptor.fragment.as_mut().unwrap().entry_point = Some("resolve_fragment".into());
    ...
    let mut interaction_descriptor = descriptor.clone();
    interaction_descriptor.vertex.entry_point = Some("background_vertex".into());
    interaction_descriptor
        .fragment
        .as_mut()
        .unwrap()
        .entry_point = Some("interaction_fragment".into());
    let interaction = cache.queue_render_pipeline(interaction_descriptor);
...
impl Buffers {
    ...
    fn append_dumps(
        ...
        let capacity = self.particles[0].size() / 32;
        if u64::from(sim.count) > capacity {
            let proposed = u64::from(sim.count)
                .max(capacity + capacity / 2)
                .min(u64::from(u32::MAX)) as u32;
            let capacity =
                if validate_counts(proposed, sim.types, &status.0.lock().unwrap()).is_ok() {
                    proposed
                } else {
                    sim.count
                };
            for (old, label) in self
                .particles
                .iter_mut()
                .zip(["particles A", "particles B"])
            {
                let next = storage(device, label, u64::from(capacity) * 32);
                context
                    .command_encoder()
                    .copy_buffer_to_buffer(old, 0, &next, 0, old.size());
                *old = next;
            }
            let sorted = storage(device, "cell-sorted particles", u64::from(capacity) * 32);
            ...
            self.sorted = sorted;
            self.neighbors = storage(device, "packed neighbor records", u64::from(capacity) * 16);
        }
        for batch in &tools.pending {
            if batch.serial <= self.last_dump {
                continue;
            }
            if batch
                .start
                .checked_add(batch.count)
                .is_none_or(|end| end > sim.count)
            {
                continue;
            }
            let data = batch.particles(sim.seed, sim.types);
            let bytes = bytemuck::cast_slice(&data);
            let upload = device.create_buffer(&BufferDescriptor {
                label: Some("particle brush upload"),
            ...
            upload.unmap();
            for destination in [&self.particles[0], &self.particles[1], &self.sorted] {
                context.command_encoder().copy_buffer_to_buffer(
                    &upload,
                    0,
                    destination,
                    u64::from(batch.start) * 32,
                    bytes.len() as u64,
                );
            }
            self.last_dump = batch.serial;
        }
        status.0.lock().unwrap().dump_ack = (sim.epoch, self.last_dump);
    }
    fn compute_group(
        &self,
        device: &RenderDevice,
...
fn simulate_and_draw(
    ...
    let Some(interaction_pipeline) = cache.get_render_pipeline(pipelines.interaction) else {
        return;
    };
    ...
    if clear_trails {
        ...
        let params = uniform(
            &device,
            &queue,
            Params::new(&sim, &appearance, 1.0, state.generation).with_tools(&tools),
        );
    ...
    state
        ...
        .as_mut()
        .unwrap()
        .append_dumps(&mut context, &device, &sim, &tools, &status);
    ...
    let pixels = size.width * size.height;
    {
        ...
        if buffers.accumulation_pixels != pixels {
            ...
            buffers.accumulation = storage(&device, "splat accumulation", u64::from(pixels) * 12);
        }
    }
    let mut display_params =
        Params::new(&sim, &appearance, alpha, state.generation).with_tools(&tools);
    ...
    }
    {
        ...
        if tools.cursor.is_some() && tools.tool != Tool::Navigate {
            pass.set_pipeline(interaction_pipeline);
            pass.draw(0..3, 0..1);
        }
    }
